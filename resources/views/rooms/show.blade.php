...
.btn-back {
  ...
  border-radius: 0px;
  cursor: pointer;
  box-shadow: 5px 5px 5px rgb(255, 175, 2);
...
.btn-back:hover {
  ...
  box-shadow: 3px 3px 0px #EFD510;
  transition-duration: .3s;
  background-color: #2C2D34;
}

.btn-back:hover .layer {
  left: 0%;
  width: 100%;
  transition-duration: .3s;
}
...
<body>
    ...
    @section('content')
    <div class="container">
        <h1 class="h1-header">Detail Kamar</h1>

        <div class="card">
            <div class="card-header">
                Kamar Nomor: {{ $room->room_number }}
            </div>
            <div class="card-body">
                @if($room->image)
                ...
                <p><strong>Harga:</strong> Rp {{ number_format($room->price, 2, '.', ',') }}</p>
